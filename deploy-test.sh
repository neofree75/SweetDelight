#!/bin/bash
# Test deploy script pre debugovanie servera

APP_NAME="SweetDelight"
APP_DIR="/var/www/SweetDelight"
PORT=5001

echo "🚀 Test deploy začína pre $APP_NAME..."
...
if [ -f "$APP_DIR/.env" ]; then
    ...
    source "$APP_DIR/.env"
    set +a
    echo "🔍 Kontrolujem env variables:"
    echo "   ERP_NEXT_URL: ${ERP_NEXT_URL:0:30}..."
    echo "   ERP_NEXT_API_KEY: ${ERP_NEXT_API_KEY:0:10}..."
    echo "   ERP_NEXT_API_SECRET: ${ERP_NEXT_API_SECRET:0:10}..."
else
...
echo "🧪 Spúšťam test servera..."
node test-server.js http://localhost:$PORT

echo "🔍 PM2 logy (posledných 20 riadkov):"
pm2 logs $APP_NAME --lines 20

echo "✅ Test deploy hotový!"
