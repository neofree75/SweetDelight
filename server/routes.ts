...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      const orderValidation = userOrderSchema.safeParse(req.body);
      ...
      try {
        ...
        customer = await erpNextService.createCustomer({
          ...
          // Pridaj delivery metódu do zákazníka
          custom_delivery_method: orderData.customerInfo.deliveryMethod || 'pickup'
        });
      ...
      try {
        console.log("Creating sales order for customer:", customer.name);
        ...
        const salesOrder = await erpNextService.createSalesOrder({
          customer: customer.name,
          customer_name: orderData.customerInfo.name,
          // Pridaj delivery informácie
          delivery_method: orderData.customerInfo.deliveryMethod || 'pickup',
          customer_address: orderData.customerInfo.address || "Dvorníky 364",
          contact_email: orderData.customerInfo.email,
          contact_mobile: orderData.customerInfo.phone,
        ...
        // 3. Vyčistiť košík po úspešnej objednávke
        const sessionId = req.sessionID;
        await storage.clearCart(sessionId);
        ...
        res.json({
          ...
          erpNextOrderId: salesOrder.name,
  ...
  app.get("/api/orders", async (req, res) => {
    try {
      ...
      const orders = await erpNextService.getCustomerOrders(userEmail);
  ...
  app.get("/api/orders/:orderId", async (req, res) => {
    try {
      ...
      const order = await erpNextService.getSalesOrder(orderId);
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      // Skús nájsť zákazníka v ERPNext podľa emailu
      const customer = await erpNextService.getCustomerByEmail(email);
      
      if (!customer) {
        return res.status(401).json({ error: "Invalid credentials" });
      }

      // Pre demo účely - skontroluj heslo (v reálnej aplikácii by malo byť zahashované)
      if (password !== "Rado1234") {
        return res.status(401).json({ error: "Invalid credentials" });
      }
